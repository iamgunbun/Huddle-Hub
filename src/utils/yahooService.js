...
export const fetchAndNormalizeYahooLeague = async (leagueId, passedUserId = null) => {
    ...
    try {
        ...
        return {
            ...
            total_rosters: parseInt(leagueData.num_teams) || 10,
            avatar: leagueData.logo_url || '/brand.png',
            platform: 'yahoo',
            settings: {
                playoff_week_start: settingsData?.playoff_start_week ? parseInt(settingsData.playoff_start_week) : 15,
                divisions: 0, 
                playoff_teams: 6,
                type: 0,
...
export const fetchAndNormalizeYahooRosters = async (leagueId, passedUserId = null) => {
    ...
    try {
        // Step 1: Get Standings (Team Names, Wins, Losses)
        ...
        if (!standingsRes.ok) return { rosters: {}, startersAndReserve: [] };
        ...
        if (Array.isArray(stLeagueObj)) {
            ...
            stTeams = stWrapper?.standings?.[0]?.teams;
        }
        ...
        // Step 2: Use the Team Keys to fetch ALL players in one matrix API call
        let teamsData = null;
