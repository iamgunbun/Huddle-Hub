...
const combThroughTransactions = async (week, currentLeagueID) => {
    ...
    // Return empty dataset for Yahoo leagues
    if (currentLeagueID && (String(currentLeagueID).includes('.') || !/^\d+$/.test(String(currentLeagueID)))) {
        return { transactionsData: [], currentSeason: new Date().getFullYear() };
    }
...
const digestTransactions = async ({transactionsData, currentSeason}) => {
    ...
    for(const transaction of transactionOrder) {
        ...
        for(const roster of digestedTransaction.rosters) {
            ...
            if (leagueTeamManagers.teamManagersMap[season]?.[roster]?.managers) {
